...
# для демонстрации работы модуля по определению треугольника смоделируем его работу - 
# функция is_triangle.
# Функция будет проверять правильность треугольника по условию -
# всякая сторона треугольника меньше суммы двух других сторон: a < b + c, b < a + c, c < a + b,
# где a > 0, b > 0, c > 0, так как длина отрезка не может быть отрицательной или равной 0.
# При тестировании на вход подаются значения в диапазоне: (0,1,2^63,2^63 + 1):
# •	для позитивного тестирования используем комбинации из наборов корректных значений в диапазоне [1, 2^63]:
# (3, 4, 5), (3,3,4), (2^63, 2^63, 1), (2^63, 2^63, 2^63). 
# •	для негативного тестирования на вход подаются комбинации значений за границами диапазона,
# а так же отрицательные числа и наборы чисел, при которых не соблюдается условия правильности треугольника:
# (-1, 0, 2^63+1), (123, 256, 123)
